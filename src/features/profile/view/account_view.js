...
export class AccountView {
    render() {
        ...
        if (!user) {
            accountSection.innerHTML = `<p class="text-muted">Please log in to view account details.</p>`;
            return;
        }
        ...
        form.addEventListener('submit', (e) => {
            ...
            if (!form.checkValidity()) {
                ...
                try {
                    UserModel.update(user.userId, updates);
                    AuthModel.signIn(email, newPassword || user.password); // Re-sign in with updated credentials
